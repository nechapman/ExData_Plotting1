...
plot3dates <- (dates >= "2007-02-01") & (dates <= "2007-02-02")
plot3_household_power_consumption <- household_power_consumption[plot3dates==TRUE,]


## Step 2 - Making Plot 3:
##------------------------
...
datetime<-strptime(paste(plot3_household_power_consumption$Date,plot3_household_power_consumption$Time),format='%d/%m/%Y %H:%M:%S')

## Clean submetering columns
Sub_metering_1 <- as.numeric(plot3_household_power_consumption$Sub_metering_1)
Sub_metering_2 <- as.numeric(plot3_household_power_consumption$Sub_metering_2)
Sub_metering_3 <- as.numeric(plot3_household_power_consumption$Sub_metering_3)
